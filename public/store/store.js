...
products = data.products || [];
    ...
    console.error(error);
    grid.innerHTML = `<p class="loading">دریافت محصولات با مشکل مواجه شد.</p>`;
  }
}
    ...
    return `
      <article class="product-card">
        <a href="product.html?id=${encodeURIComponent(product.id)}">
          <img class="product-image" src="${escapeHtml(image)}" alt="${title}" loading="lazy">
        </a>
        <div class="product-content">
          <h3 class="product-title">${title}</h3>
          <div class="product-price">${price}</div>
          <button class="product-button" type="button" onclick="addToCart('${escapeAttribute(product.id)}')">افزودن به سبد خرید</button>
        </div>
      </article>`;
  }).join("");
}
...
function formatPrice(price) {
  ...
  if (Number.isNaN(number)) return escapeHtml(String(price));
  return `${number.toLocaleString("fa-IR")} تومان`;
}

function getCart() {
  try { return JSON.parse(localStorage.getItem("apadana_cart")) || []; }
  catch { return []; }
}
...
function addToCart(productId) {
  ...
  if (existing) {
    ...
    cart.push({
      ...
      image: product.image || "../assets/placeholder.svg", quantity: 1
    });
  }
  saveCart(cart);
