...
if (!response.ok) {
  if (response.status === 401) {
    ...
    alert(data.message || "دریافت سفارش‌ها انجام نشد.");
  }
...
return `
  <div class="order-row">
    ...
    <div class="order-main">
      ...
      <strong>
        سفارش شماره ${Number(order.id).toLocaleString("fa-IR")}
      </strong>
      ...
      <div>
        وضعیت:
        <span class="order-status status-${escapeAttribute(order.status)}">
          ${status}
        </span>
      </div>
    ...
    <button
      type="button"
      class="secondary-button"
      onclick="showOrderDetails('${escapeAttribute(order.id)}')"
    >
      جزئیات
    </button>
