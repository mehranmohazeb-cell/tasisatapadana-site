async function handleStoreApi(request, env) {
    ...
    try {
      const ordersResult = await env.DB
        .prepare(
          ...
          "customer_address, total, status, created_at " +
          "FROM orders " +
          "ORDER BY id DESC"
        )
        .all();
      ...
      for (const order of orders) {
        const itemsResult = await env.DB
          .prepare(
            ...
            "product_name, price, quantity, created_at " +
            "FROM order_items " +
            "WHERE order_id = ? " +
            "ORDER BY id ASC"
          )
        ...
        return Response.json(
          {
            ...
            error: "INVALID_DATA",
            message:
              "قیمت و موجودی باید عدد صحیح صفر یا بیشتر باشند.",
          },
          { status: 400 }
        );
      }
  ...
  if (
    ...
    try {
      ...
      if (Array.isArray(body.images)) {
        ...
        for (
          let i = 0;
          i < body.images.length;
          i++
        ) {
          const imagePath =
            String(body.images[i] || "").trim();
  ...
  if (
    ...
    try {
      ...
      const images =
        await getProductImages(result.id);
  ...
  if (
    ...
    try {
          ...
          return Response.json(
            {
              ...
              error: "INVALID_ITEM",
              message:
                "اطلاعات یکی از کالاهای سفارش نامعتبر است.",
            },
            { status: 400 }
          );
        }
          ...
          return Response.json(
            {
              ...
              message:
                "یکی از کالاهای سفارش دیگر قابل سفارش نیست.",
            },
            { status: 400 }
          );
        }
          ...
          return Response.json(
            {
              ...
              message:
                `موجودی «${product.name}» برای تعداد درخواستی کافی نیست.`,
            },
            { status: 400 }
          );
        }
      ...
      if (!orderId) {
        throw new Error(
          "ORDER_ID_NOT_CREATED"
        );
      }

      // ثبت کالاهای سفارش و کاهش موجودی
  ...
  if (
    ...
    return Response.json({
      ...
      status: "ready",
      message:
        "Checkout infrastructure is prepared.",
    });
  }
